...
describe('getRamadanDay', () => {
  it('returns 1 on the first day of Ramadan', () => {
    expect(getRamadanDay(new Date(2026, 1, 28), RAMADAN_START)).toBe(1)
  })

  it('returns 15 on the 15th day', () => {
    expect(getRamadanDay(new Date(2026, 2, 14), RAMADAN_START)).toBe(15)
  })

  it('returns 30 on the last day', () => {
    expect(getRamadanDay(new Date(2026, 2, 29), RAMADAN_START)).toBe(30)
  })

  it('returns 0 before Ramadan', () => {
    expect(getRamadanDay(new Date(2026, 1, 27), RAMADAN_START)).toBe(0)
  })

  it('returns 0 after Ramadan', () => {
    expect(getRamadanDay(new Date(2026, 2, 30), RAMADAN_START)).toBe(0)
  })
})

describe('getDaysUntilRamadan', () => {
  it('returns correct days before Ramadan', () => {
    expect(getDaysUntilRamadan(new Date(2026, 1, 16), RAMADAN_START)).toBe(12)
  })

  it('returns 0 on Ramadan start', () => {
    expect(getDaysUntilRamadan(new Date(2026, 1, 28), RAMADAN_START)).toBe(0)
  })

  it('returns 0 during Ramadan', () => {
    expect(getDaysUntilRamadan(new Date(2026, 2, 10), RAMADAN_START)).toBe(0)
  })

  it('returns 1 on the day before Ramadan', () => {
    expect(getDaysUntilRamadan(new Date(2026, 1, 27), RAMADAN_START)).toBe(1)
  })
})
...
describe('isBeforeRamadan', () => {
  ...
  it('returns false on Ramadan start', () => {
    expect(isBeforeRamadan(new Date(2026, 1, 28), RAMADAN_START)).toBe(false)
  })

  it('returns false during Ramadan', () => {
    expect(isBeforeRamadan(new Date(2026, 2, 10), RAMADAN_START)).toBe(false)
  })
})

describe('isDuringRamadan', () => {
  it('returns true on first day', () => {
    expect(isDuringRamadan(new Date(2026, 1, 28), RAMADAN_START)).toBe(true)
  })

  it('returns true on last day (day 30)', () => {
    expect(isDuringRamadan(new Date(2026, 2, 29), RAMADAN_START)).toBe(true)
  })

  it('returns false before Ramadan', () => {
    expect(isDuringRamadan(new Date(2026, 1, 27), RAMADAN_START)).toBe(false)
  })

  it('returns false after Ramadan', () => {
    expect(isDuringRamadan(new Date(2026, 2, 30), RAMADAN_START)).toBe(false)
  })
})

describe('isAfterRamadan', () => {
  it('returns false during Ramadan', () => {
    expect(isAfterRamadan(new Date(2026, 2, 10), RAMADAN_START)).toBe(false)
  })

  it('returns true after Ramadan', () => {
    expect(isAfterRamadan(new Date(2026, 2, 30), RAMADAN_START)).toBe(true)
  })
})
